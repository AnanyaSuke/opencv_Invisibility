...
# initializing of the camera
cap = cv2.VideoCapture(0)
bars = cv2.namedWindow("bars")  # 6 bars created here
# trackbars for color adjustment
cv2.createTrackbar("upper_hue", "bars", 100, 180, hello)
...
while (True):
    ...
    # check if the frame is returned from brake
    if (ret):
        break
...
while (True):
    ...
    # getting the HSV values for the masking the cloak
    ...
    Area_of_cloak = cv2.merge((b,g,r))

    final = cv2.bitwise_or(frame_inv, Area_of_cloak)
